 ...
 * @since 1.0-dev
 */
public class FieldRenderer extends ComponentRenderer<Field> {
    ...
    @Override
    void renderInternal(Field input, float offsetX, float offsetY) {
        ...
        if (input.getSubComponentsHandler().getComponents() != null) {
            for (Component c : input.getSubComponentsHandler().getComponents().values()) {
                this.engine.getRenderers().getUiRenderer().renderSubComponent(c.getClass(), c.getId(), input.getId());
            }
        }
    }
}
